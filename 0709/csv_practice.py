...
def signin(id, pw) :
	f = open('0709/data.csv', 'r', encoding='utf-8')
	data = csv.reader(f)
	print("=============================")
	for i in data :
		if id == data[i][0] :
...
def add_user() :
	...
	pw_input = input("패스워드 입력 : ")
	new_user = {'id' : id_input, 'pw' : pw_input}

	with open('0709/data.csv','w') as f:
		w = csv.writer(f)
		w.writerow(new_user.keys())
		w.writerow(new_user.values())
		f.close()
